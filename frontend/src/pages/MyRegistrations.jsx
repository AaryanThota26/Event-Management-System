...
const MyRegistrations = () => {
  ...
  return (
    <div className="min-h-screen bg-surface-bright">
      {/* Skip to main */}
      <a href="#registrations-main" className="skip-to-main">
        Skip to my registrations
      </a>
      ...
      <header className="sticky top-0 z-50 flex justify-between items-center w-full px-lg py-sm bg-surface border-b border-outline-variant shadow-sm">
        <div className="flex items-center gap-xl">
          ...
          <nav className="hidden md:flex items-center gap-lg" aria-label="Main navigation">
            <a className="text-on-surface-variant hover:text-primary transition-colors font-label-md" href={DASHBOARD_ROUTES[user?.role] || '/user/dashboard'}>Dashboard</a>
            <a className="text-on-surface-variant hover:text-primary transition-colors font-label-md" href="/events">Events</a>
            <a className="text-primary border-b-2 border-primary pb-1 font-medium font-label-md" href="/user/registrations" aria-current="page">My Registrations</a>
          </nav>
        </div>
      </header>

      <main id="registrations-main" className="max-w-[1440px] mx-auto p-lg sm:p-xl">
        {/* Hero */}
        ...
        <div className="min-h-[400px]">
          ...
          {!loading && !error && registrations.length === 0 && (
            <div className="flex flex-col items-center justify-center min-h-[400px]">
              ...
              <p className="font-headline-sm text-headline-sm text-on-surface mb-xs">No registrations yet</p>
              <p className="text-body-sm text-on-surface-variant mb-lg text-center max-w-md">
                You have not registered for any events yet. Browse available events to find something interesting.
              </p>
              <Link
                to="/events"
              ...
              {registrations.map((reg) => (
                ...
                >
                  <div className="flex items-start justify-between mb-md">
                    ...
                    <span className="material-symbols-outlined text-primary text-[20px] mt-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" aria-hidden="true">
                      arrow_forward
                    </span>
                  </div>
